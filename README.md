...
- **前端闲鱼风格化升级**：
  - 新增设计 Token 体系（`--xy-bg`, `--xy-brand-500`, `--xy-shadow` 等 CSS 变量）
  - 新增 4 个核心业务页面：详情页（Detail）、发布页（Publish）、聊天页（Chat）、订单页（Orders）
  - 首页重构：「探索 + 行动入口」双层结构，减少决策负担
  - 统一视觉语言：卡片阴影、圆角体系、品牌橙色主题、入场动效
  - 新增 `docs/frontend-design-token-page-spec-mapping.md` 设计规范文档
- **测试环境固化（ENVLOCK）**：
  - 统一使用 Python 3.12 + `.venv312` 虚拟环境，避免多版本解释器漂移
  - 更新 `setup.sh`：自动检测 Python 3.12、固定依赖安装入口
  - 更新 `requirements.txt` / `requirements.lock`：pytest 8.4.2、pytest-asyncio 0.26.0
  - README 测试入口更新为：`bash scripts/qa/lock_test_env.sh -q tests/`
- **阶段门禁与证据包**：
  - 新增阶段汇总报告 `docs/reports/xy-wave-gate-progress-20260305.md`
  - 新增 3 组证据包：W26 环境指纹、W28 E-M1/E-M2、W34 G2
  - 涵盖：commit SHA、git diff、非 mock 链路日志、QA 回执绑定
