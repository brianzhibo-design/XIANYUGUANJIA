...
| 模块 | 名称 | 当前状态 | 目标里程碑 |
|---|---|---|---|
| M1 | 跨平台启动器 Launcher | 🟡 进行中 | 一键启动/停止/恢复（Mac/Windows） |
| M2 | 安装与环境诊断 Doctor | 🟡 进行中 | 小白首启 2 分钟内完成体检 |
| M3 | 新手引导 UI | 🟡 进行中 | 不懂代码用户可完成首启与首单 |
| M4 | 配置与密钥管理 | 🟡 进行中 | 配置热更新、敏感信息本地保护 |
| M5 | WS Runtime | ✅ 可用 | ACK/心跳/重连稳定 |
| M6 | Message Pipeline | 🟡 进行中 | 统一消息编排与回退 |
| M7 | Context/Dedup | ✅ 可用 | 幂等去重与会话记忆 |
| M8 | Intent/Extract | 🟡 进行中 | 询价字段提取稳定 |
| M9 | Quote/Bargain | 🟡 进行中 | 自动改价执行链封板 |
| M10 | Compliance/Safety | ✅ 可用 | 合规拦截与审计追溯 |
| M11 | Metrics/Recover | 🟡 进行中 | 运营指标+一键恢复 |
| M12 | SQLite Repo | ✅ 可用 | 本地持久化与回滚 |

### 路线图分期（详细）

#### Phase A（W1-W2）MVP 封板
- 完成 M1/M2/M4/M5/M6/M7/M9/M10/M12 的稳定闭环
- 验收标准：询价→报价/议价→催付→履约最小闭环可稳定运行

#### Phase B（W3-W4）可运营化
- 完成 M3/M8/M11 与回调事件补偿
- 验收标准：新手可独立部署，异常可定位可恢复

#### Phase C（W5-W6）企业化增强
- 完成售后路由产品化、运营看板深化、成本治理（token/人工接管率）
- 验收标准：周报可直接反映 ROI 与运营质量

### 企业级门禁（每阶段必须通过）
1. 架构评审（接口契约、数据模型、回滚策略）
2. 质量门禁（功能/稳定/安全/易用性）
3. 灰度发布（小流量→全量）
4. 复盘闭环（RCA + 改进项入库）

### 风险与人工边界（明确）
- AI 不独立处理：电话联系客户、转账退款、高风险纠纷
- 超边界动作统一进入人工接管队列
