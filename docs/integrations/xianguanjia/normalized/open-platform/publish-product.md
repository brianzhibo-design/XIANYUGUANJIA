...
## raw_html_path
`/Users/brianzhibo/openclaw/xianyu-openclaw/docs/external/xianguanjia/fullcrawl/raw/8f87a4bfc40b3213.html`
...
| timestamp | string | 是 | 秒级时间戳（Query） |
| sign | string | 是 | MD5 签名（Query） |
| seller_id | string | 否 | 商务对接商家 ID |
| product_id | int64 | 是 | 管家商品 ID |
| user_name | string[] | 是 | 上架到的闲鱼会员名列表 |
| specify_publish_time | string | 否 | 定时上架时间（`yyyy-MM-dd HH:mm:ss`） |
| notify_url | string | 否 | 上架结果回调地址 |
...
| msg | string | 状态信息 |
| data | object | 受理结果；实际上架结果以回调通知为准 |
